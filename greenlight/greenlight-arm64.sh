...
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $pro/$appname/extra/icon.png https://github.com/trashbus99/profork/raw/master/greenlight/extra/icon.png; chmod a+x $dep/* 2>/dev/null; cd ~/
...
echo
sleep 0.33
echo -e "${X}THIS WILL INSTALL GREENLIGHT FOR BATOCERA"
...
echo; #echo -e "${W}- - -"
echo;
echo -e "${W}THIS WILL INSTALL GREENLIGHT FOR BATOCERA"
...
# --------------------------------------------------------------------
mkdir greenlight 2>/dev/null
tar -xzf $temp/*.tar.gz -C $temp/greenlight
cd $temp/greenlight/*/
mv ./* $pro/$appname/
chmod a+x $pro/$appname/greenlight* 2>/dev/null
...
# --------------------------------------------------------------------
# -- prepare Ports file, 
port="/userdata/roms/ports/Greenlight.sh" 
rm $port 2>/dev/null
echo '#!/bin/bash ' >> $port
echo 'killall -9 greenlight && batocera-mouse show' >> $port
echo '/userdata/system/pro/greenlight/Launcher' >> $port
dos2unix $port 
chmod a+x $port 
# --------------------------------------------------------------------
# -- get padtokey profile
rm "/userdata/roms/ports/Greenligh.sh.keys" 
wget -q -O "/userdata/roms/ports/Greenlight.sh.keys" "https://github.com/trashbus99/profork/raw/master/xcloud/extra/xcloud.sh.keys"
# --------------------------------------------------------------------
