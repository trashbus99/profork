...
#       DEFINE APP INFO >> 
APPNAME=luna 
APPLINK=https://github.com/trashbus99/profork/releases/download/r1/luna.aarch64.tar.gz
...
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $pro/$appname/extra/icon.png https://github.com/trashbus99/profork/raw/master/amazonluna/extra/icon.png; chmod a+x $dep/* 2>/dev/null; cd ~/
...
# --------------------------------------------------------------------
mkdir luna 2>/dev/null
tar -xzf $temp/*.tar.gz -C $temp/luna
cd $temp/luna/*/
mv ./* $pro/$appname/
chmod a+x $pro/$appname/luna* 2>/dev/null
...
echo '/userdata/system/pro/luna/Launcher' >> $port
dos2unix $port 
chmod a+x $port 
# --------------------------------------------------------------------
