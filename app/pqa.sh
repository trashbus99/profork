...
if [ ! -f /userdata/system/pro ]; then
    bash /userdata/system/quiz.sh
    if [ ! -f /userdata/system/pro ]; then
        echo "Quiz not passed. Exiting launcher."
        exit 1
    fi
fi

# Launch secret ARM64 menu
...
if [ $score -eq $total ]; then
    ...
    dialog --msgbox "Not bad!\n\nScore: $score/$total\nYou're not completely lost in SBC delusion, but you need to study Mali coping rituals a bit more.\nTry again to unlock the tools." 15 60
else
    if command -v cvlc >/dev/null 2>&1; then
    ...
    dialog --msgbox "💀 WOMP WOMP.\n\nScore: $score/$total\nYou Pi'd when you should’ve Mini PC’d.\n\nThis is not your time. Try again." 15 60
fi

# Optional: mockery before loop
echo -n "Analyzing coping levels"
for i in {1..5}; do echo -n "."; sleep 0.3; done
echo -e "\nDiagnosis: Terminal SBC Delusion"
sleep 2
echo "Suggesting: Used OptiPlex. Steam Deck. N100. Therapy."
sleep 2
echo "Rewinding time..."
sleep 1
