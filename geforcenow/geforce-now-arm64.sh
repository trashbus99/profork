...
#       DEFINE APP INFO >> 
APPNAME=geforcenow 
APPLINK=https://github.com/trashbus99/profork/releases/download/r1/geforcenow.aarch64.tar.gz
...
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $pro/$appname/extra/icon.png https://github.com/trashbus99/profork/raw/master/geforcenow/extra/icon.png; chmod a+x $dep/* 2>/dev/null; cd ~/
...
echo
sleep 0.33
echo -e "${X}THIS WILL INSTALL GEFORCE NOW FOR BATOCERA"
...
echo; #echo -e "${W}- - -"
echo;
echo -e "${W}THIS WILL INSTALL GEFORCE NOW FOR BATOCERA"
...
# --------------------------------------------------------------------
mkdir geforcenow 2>/dev/null
tar -xzf $temp/*.tar.gz -C $temp/geforcenow
cd $temp/geforcenow/*/
mv ./* $pro/$appname/
chmod a+x $pro/$appname/geforcenow* 2>/dev/null
...
# --------------------------------------------------------------------
# -- prepare Ports file, 
port="/userdata/roms/ports/Geforce-Now.sh" 
rm $port 2>/dev/null
echo '#!/bin/bash ' >> $port
echo 'killall -9 geforcenow && unclutter-remote -s' >> $port
echo '/userdata/system/pro/geforcenow/Launcher' >> $port
dos2unix $port 
chmod a+x $port 
# --------------------------------------------------------------------
