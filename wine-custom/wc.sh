...
# Advice:
#   - Use wtgz (TGZ) for small games that make large writes,
#     so that the game can be quickly repackaged (the file will be renamed *.wtgz).
#   - Use wsquashfs (SquashFS) for larger games that make small writes (e.g., only save files),
#     as repackaging would take too long or wear down storage hardware.
#   Note: Compressed folders are read-only. Batocera handles writes by either unzipping (TGZ)
#         or creating a new directory for differences (SquashFS).
