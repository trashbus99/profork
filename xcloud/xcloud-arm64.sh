...
#       DEFINE APP INFO >> 
APPNAME=xcloud 
APPLINK=https://github.com/trashbus99/profork/releases/download/r1/xcloud.aarch64.tar.gz
APPHOME=github.com/trashbus99/PROFORK
#---------------------------------------------------------------------
#       DEFINE LAUNCHER COMMAND >>
COMMAND='mkdir /userdata/system/pro/'$APPNAME'/home 2>/dev/null; mkdir /userdata/system/pro/'$APPNAME'/config 2>/dev/null; mkdir /userdata/system/pro/'$APPNAME'/roms 2>/dev/null; LD_LIBRARY_PATH="/userdata/system/pro/.dep:${LD_LIBRARY_PATH}" HOME=/userdata/system/pro/'$APPNAME'/home XDG_CONFIG_HOME=/userdata/system/pro/'$APPNAME'/config QT_SCALE_FACTOR="1" GDK_SCALE="1" XDG_DATA_HOME=/userdata/system/pro/'$APPNAME'/home DISPLAY=:0.0 /userdata/system/pro/'$APPNAME'/xcloud --no-sandbox "${@}"'
#--------------------------------------------------------------------- 
######################################################################
######################################################################
######################################################################
...
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $pro/$appname/extra/icon.png https://github.com/trashbus99/profork/raw/master/xcloud/extra/icon.png; chmod a+x $dep/* 2>/dev/null; cd ~/
...
echo
sleep 0.33
echo -e "${X}THIS WILL INSTALL XCLOUD FOR BATOCERA"
...
echo; #echo -e "${W}- - -"
echo;
echo -e "${W}THIS WILL INSTALL XCLOUD FOR BATOCERA"
...
# --------------------------------------------------------------------
mkdir xcloud 2>/dev/null
tar -xzf $temp/*.tar.gz -C $temp/xcloud
cd $temp/xcloud/*/
mv ./* $pro/$appname/
chmod a+x $pro/$appname/xcloud* 2>/dev/null
...
# --------------------------------------------------------------------
# -- prepare Ports file, 
port="/userdata/roms/ports/xcloud.sh" 
rm $port 2>/dev/null
echo '#!/bin/bash ' >> $port
echo 'killall -9 xcloud && unclutter-remote -s' >> $port
echo '/userdata/system/pro/xcloud/Launcher' >> $port
dos2unix $port 
chmod a+x $port 
# --------------------------------------------------------------------
# -- get padtokey profile
rm "/userdata/roms/ports/xcloud.sh.keys" 
wget -q -O "/userdata/roms/ports/xcloud.sh.keys" "https://github.com/trashbus99/profork/raw/master/xcloud/extra/xcloud.sh.keys"
# --------------------------------------------------------------------
