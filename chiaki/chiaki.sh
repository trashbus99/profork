...
APPLINK=$(curl -s https://api.github.com/repos/streetpea/chiaki-ng/releases/latest | jq -r '.assets[] | select(.name | endswith("AppImage_x86_64")) | .browser_download_url')
...
echo 'LD_LIBRARY_PATH="/userdata/system/pro/.dep:${LD_LIBRARY_PATH}" DISPLAY=:0.0 /userdata/system/pro/'$appname'/'$appname'.AppImage --no-sandbox' >> $port
dos2unix $port
chmod a+x $port
