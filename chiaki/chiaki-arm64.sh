...
dialog --title "X-Minecraft Launcher" --yesno "Chiaki is Experimental on Batocera. Some device vulkan drivers are incompatible (e.g. orange pi 5 pro)\n\nContinue installing?" 8 50
...
APPLINK=$(curl -s https://api.github.com/repos/streetpea/chiaki-ng/releases/latest | jq -r '.assets[] | select(.name | endswith("AppImage_arm64")) | .browser_download_url')
APPHOME="GITHUB/STREETPEA"
#---------------------------------------------------------------------
