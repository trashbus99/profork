...
# --------------------------------------------------------------------
# --------------------------------------------------------------------
APPNAME=LUANTI # for installer info
appname=luanti # directory name in /userdata/system/pro/...
AppName=$appname # App.AppImage name
APPPATH=/userdata/system/pro/$appname/$appname.AppImage
#APPLINK=http://PROFORK/app/$appname.AppImage
APPLINK=$(curl -s https://api.github.com/repos/rollerozxa/luanti-appimage/releases/latest | jq -r '.assets[] | select(.name | endswith("x86_64.AppImage")) | .browser_download_url')
ORIGIN="rollerozxa/lunati-appimage" # credit & info
# --------------------------------------------------------------------
# --------------------------------------------------------------------
# --------------------------------------------------------------------
# --------------------------------------------------------------------
...
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $pro/$appname/extra/icon.png https://github.com/trashbus99/profork/blob/master/luanti/extra/luanti.png; chmod a+x $dep/* 2>/dev/null; cd ~/
...
echo 'LD_LIBRARY_PATH="/userdata/system/pro/.dep:${LD_LIBRARY_PATH}" DISPLAY=:0.0 /userdata/system/pro/'$appname'/'$appname'.AppImage   "$@"' >> /userdata/roms/ports/Luanti.sh
chmod +x /userdata/roms/ports/Luanti.sh
######################################################################
######################################################################
######################################################################
######################################################################
...
wget -q -O $icon https://github.com/trashbus99/profork/raw/master/luanti/extra/luanti.png
# //
