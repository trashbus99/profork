...
# --------------------------------------------------------------------
# --------------------------------------------------------------------
APPNAME="HANDBRAKE" # for installer info
appname=handbrake # directory name in /userdata/system/pro/...
AppName=handbrake # App.AppImage name
APPPATH=/userdata/system/pro/$appname/$AppName.AppImage
APPLINK=$(curl -s https://api.github.com/repos/ivan-hc/Handbrake-AppImage/releases/tags/continuous | jq -r ".assets[] | select(.name | endswith(\".AppImage\")) | .browser_download_url")
ORIGIN="github.com/ivan-hc/Handbrake-AppImage"  # credit & info where it fetches the latest release
...
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $pro/$appname/extra/icon.png https://github.com/trashbus99/profork/raw/master/handbrake/extra/handbrake.png; chmod a+x $dep/* 2>/dev/null; cd ~/
...
######################################################################
######################################################################
echo 'LD_LIBRARY_PATH=/userdata/system/pro/.dep:${LD_LIBRARY_PATH}  DISPLAY=:0.0 /userdata/system/pro/handbrake/handbrake.AppImage ${@}' >> $launcher
######################################################################
######################################################################
######################################################################
######################################################################
...
wget -q -O $icon https://github.com/trashbus99/profork/raw/master/handbrake/extra/handbrake.png
# //
