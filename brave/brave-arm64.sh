...
wget --tries=10 --no-check-certificate --no-cache --no-cookies -q -O $pro/$appname/extra/icon.png https://github.com/trashbus99/profork/raw/master/brave/extra/icon.png; chmod a+x $dep/* 2>/dev/null; cd ~/
...
echo 'LD_LIBRARY_PATH="/userdata/system/pro/.dep:${LD_LIBRARY_PATH}" DISPLAY=:0.0 /userdata/system/pro/'$appname'/'$appname'.AppImage  --no-sandbox --disable-gpu --test-type "$@"' >> $launcher
######################################################################
######################################################################
######################################################################
######################################################################
...
wget -q -O $icon https://github.com/trashbus99/profork/raw/master/brave/extra/icon.png
# //
